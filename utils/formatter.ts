...
export const formatConstructionUnit = (
    ...
    normalDecimalPlaces: number = 6,
    engineeringDecimalPlaces: number = 5
): FormattedValue => {
    ...
    if (isUnitless) {
        ...
        const val = parseFloat(decimalFeet.toFixed(normalDecimalPlaces));
    ...
    if (primaryDimension > 1 && !isUnitless) {
        ...
        const formattedVal = parseFloat(val.toFixed(engineeringDecimalPlaces));
        const dimensionLabel = `${primaryDimension === 2 ? 'SQUARE' : 'CUBIC'} ${displayUnit.toUpperCase()}`;
        ...
        return {
            ...
            denominator: 0,
            isNegative,
            showFeetLabel: false, // Don't show unit labels in the main display
            showInchLabel: false,
            showYardLabel: false,
            showDash: false,
            inputBuffer: '',
    ...
    // Fraction display precision is a user preference for engineering measurements.
    // Calculations remain decimal-feet internally; only the displayed fraction is rounded.
    const precision = fractionDenominator;
    ...
    if (convertedUnit) {
        ...
        const formattedNum = parseFloat(val.toFixed(engineeringDecimalPlaces));
...
export const builderToDisplay = (builder: BuilderState, buffer: string, preferredUnit?: 'feet' | 'inch' | 'yard'): FormattedValue => {
    ...
    const activeUnit = builder.yard !== null ? 'YARD' : (builder.inch !== null ? 'INCH' : (builder.feet !== null ? 'FEET' : (preferredUnit || 'FEET').toUpperCase()));
    const dimensionLabel = builder.dimension > 1 && (builder.yard !== null || builder.feet !== null || builder.inch !== null)
        ? `${builder.dimension === 2 ? 'SQUARE' : 'CUBIC'} ${activeUnit}`
        : undefined;
