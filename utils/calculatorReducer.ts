...
export enum CalculatorActionType {
    ...
    TAPE_RECALL = 'TAPE_RECALL',
    TAPE_CLEAR = 'TAPE_CLEAR',
}
...
export type CalculatorAction =
    ...
    | { type: CalculatorActionType.MEMORY_STORE }
    | { type: CalculatorActionType.MEMORY_RECALL }
    | { type: CalculatorActionType.MEMORY_ADD }
    | { type: CalculatorActionType.MEMORY_SUBTRACT }
    | { type: CalculatorActionType.MEMORY_CLEAR }
    | { type: CalculatorActionType.SET_DIMENSION; payload: 1 | 2 | 3 }
    | { type: CalculatorActionType.TAPE_RECALL; payload: number }
    | { type: CalculatorActionType.TAPE_CLEAR };
...
export const initialCalculatorState: CalculatorState = {
    ...
    memoryDimension: 1,
    memoryIsUnitless: true,
    memoryPreferredUnit: 'feet',
    tape: [],
...
export const calculatorReducer = (state: CalculatorState, action: CalculatorAction): CalculatorState => {
    switch (action.type) {
        ...
        case CalculatorActionType.CONVERSION: {
            if (state.convertedUnit) {
                const units: Array<'feet' | 'inch' | 'yard'> = ['feet', 'inch', 'yard'];
                const nextUnit = units[(units.indexOf(state.convertedUnit) + 1) % units.length];
                return { ...state, isConversionMode: true, convertedUnit: nextUnit, preferredUnit: nextUnit };
            }
            return {
                ...state,
        ...
        case CalculatorActionType.SET_DIMENSION: {
            const dimension = action.payload;
            const hasInput = state.builder.feet !== null || state.builder.inch !== null || state.builder.yard !== null || state.builder.numerator !== null || state.inputBuffer !== '';
            return {
                ...state,
                builder: { ...state.builder, dimension },
                activeDimension: dimension,
                // Choosing SQ/CB establishes a construction unit mode before the first number is typed.
                isUnitless: dimension === 1 ? (hasInput ? state.isUnitless : true) : (hasInput ? state.isUnitless : false),
        ...
        case CalculatorActionType.EQUAL: {
            ...
            if (resultDim === 2) {
                ...
                // Mixed feet + inch construction entries (e.g. 3 FEET 3 INCH)
                // are presented as square feet, matching common engineering practice.
                if (state.builder.feet === null && state.builder.inch !== null && state.builder.yard === null) {
                    autoConvertedUnit = 'inch';
                } else if (state.builder.feet === null && state.builder.yard !== null && state.builder.inch === null) {
        ...
        case CalculatorActionType.CLEAR: {
            // Clear the active calculation, but keep the construction-style memory register.
            return {
                ...initialCalculatorState,
                memory: state.memory,
                memoryHasValue: state.memoryHasValue,
                memoryDimension: state.memoryDimension,
                memoryIsUnitless: state.memoryIsUnitless,
                memoryPreferredUnit: state.memoryPreferredUnit,
                tape: state.tape,
            };
        }

        case CalculatorActionType.MEMORY_STORE: {
            const hasNewInput = state.builder.feet !== null || state.builder.inch !== null || state.builder.yard !== null || state.builder.numerator !== null || state.inputBuffer !== '';
            return {
                ...state,
                memory: hasNewInput ? convertBuilderToDecimal(state.builder, state.inputBuffer) : state.displayValue,
                memoryHasValue: true,
                memoryDimension: hasNewInput ? state.builder.dimension : state.activeDimension,
                memoryIsUnitless: hasNewInput ? isBuilderUnitless(state.builder) : state.isUnitless,
                memoryPreferredUnit: state.preferredUnit,
            };
        }
        ...
        case CalculatorActionType.MEMORY_ADD: {
            const hasNewInput = state.builder.feet !== null || state.builder.inch !== null || state.builder.yard !== null || state.builder.numerator !== null || state.inputBuffer !== '';
            const value = hasNewInput ? convertBuilderToDecimal(state.builder, state.inputBuffer) : state.displayValue;
            return {
                ...state,
        ...
        case CalculatorActionType.MEMORY_SUBTRACT: {
            const hasNewInput = state.builder.feet !== null || state.builder.inch !== null || state.builder.yard !== null || state.builder.numerator !== null || state.inputBuffer !== '';
            const value = hasNewInput ? convertBuilderToDecimal(state.builder, state.inputBuffer) : state.displayValue;
            return {
                ...state,
        ...
        case CalculatorActionType.MEMORY_CLEAR:
            return { ...state, memory: 0, memoryHasValue: false, memoryDimension: 1, memoryIsUnitless: true, memoryPreferredUnit: 'feet' };

        case CalculatorActionType.TAPE_RECALL: {
            const entry = state.tape[action.payload];
        ...
        case CalculatorActionType.TAPE_CLEAR:
            return { ...state, tape: [] };
